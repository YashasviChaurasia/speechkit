...
def inspect_media(path: Path, ffprobe: str = "ffprobe") -> MediaInfo:
    ...
    except (OSError, subprocess.SubprocessError, json.JSONDecodeError) as e: raise MediaError("We could not read this file as audio or video. Re-export it as a complete WAV, MP3, M4A, MP4, MOV, MKV, or WebM file.") from e
    ...
    if not any(isinstance(stream, dict) and stream.get("codec_type")=="audio" for stream in streams): raise MediaError("This video has no audio track. Choose a recording with clear spoken audio.")
    try:
        duration = float(format_data["duration"])
    except (KeyError, TypeError, ValueError) as e:
        raise MediaError("This recording has no playable duration. Re-export the source file and try again.") from e
    if not math.isfinite(duration) or duration <= 0:
        raise MediaError("This recording has no playable duration. Re-export the source file and try again.")
...
def extract_audio(source: Path, target: Path, ffmpeg: str = "ffmpeg") -> Path:
    ...
    except (OSError, subprocess.SubprocessError) as e: raise MediaError("We found media but could not extract its audio. Re-export the recording, then try again.") from e
